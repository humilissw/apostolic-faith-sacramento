...
def upgrade():
    ...
    op.add_column('user', sa.Column('new_id', mysql.VARCHAR(length=36), default=sa.text('UUID()')))
    op.add_column('item', sa.Column('new_id', mysql.VARCHAR(length=36), default=sa.text('UUID()')))
    op.add_column('item', sa.Column('new_owner_id', mysql.VARCHAR(length=36), nullable=True))
