...
router = Blueprint("login", __name__, url_prefix="")


def _cookie_kwargs(req):
    scheme = req.url_scheme
    secure = scheme == "https"
        ...
        jsonify(
            Token(
                ...
                refresh_token_expires=int(
                    (refresh_expires - datetime.now(timezone.utc)).total_seconds()
                ),
                scopes=token_scopes,
            ).model_dump()
        )
...
def refresh_token():
    ...
    result = session.execute(
        select(RefreshToken).where(
            RefreshToken.token == body.refresh_token, RefreshToken.revoked != True
        )
    )
    ...
    new_stored = RefreshToken(
        user_id=user.id, token=new_refresh_token, expires_at=new_refresh_expires
    )
...
def revoke_token():
    ...
    result = session.execute(
        select(RefreshToken).where(
            RefreshToken.token == body.token, RefreshToken.user_id == current_user.id
        )
    )
    stored = result.scalar_one_or_none()
    if stored:
    ...
    try:
        ...
        if payload and payload.get("sub") == current_user.email:
            session.execute(
                update(RefreshToken)
                .where(RefreshToken.user_id == current_user.id, RefreshToken.revoked != True)
                .values(revoked=True)
            )
            session.commit()
...
def client_credentials_login():
    ...
    result = session.execute(
        select(ClientCredentials).where(
            ClientCredentials.client_id == client_id, ClientCredentials.is_active == True
        )
    )
    ...
    result = session.execute(
        select(RefreshToken).where(RefreshToken.user_id == "client:" + client_id)
    )
    ...
    if existing:
        ...
        db_refresh = RefreshToken(
            user_id="client:" + client_id, token=refresh_token_str, expires_at=refresh_expires
        )
        session.add(db_refresh)
    session.commit()
    resp = make_response(
...
def authorization_code():
    ...
    result = session.execute(
        select(ClientCredentials).where(
            ClientCredentials.client_id == body.client_id, ClientCredentials.is_active == True
        )
    )
    client = result.scalar_one_or_none()
    if client is None:
    ...
    return jsonify(
        AuthorizationCodeResponse(
            access_token=auth_code, token_type="code", expires_in=600
        ).model_dump()
    )
...
def authorization_code_token():
    ...
    session.commit()
    result = session.execute(
        select(RefreshToken).where(RefreshToken.user_id == "client:" + body.client_id)
    )
    existing = result.scalar_one_or_none()
    if existing:
        existing.token = refresh_token_str
        ...
        db_refresh = RefreshToken(
            user_id="client:" + body.client_id, token=refresh_token_str, expires_at=refresh_expires
        )
        session.add(db_refresh)
    session.commit()
    resp = make_response(
...
def implicit_token():
    ...
    return jsonify(
        ImplicitTokenResponse(
            access_token=access_token, token_type="bearer", expires_in=access_expires
        ).model_dump()
    )
