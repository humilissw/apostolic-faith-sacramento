...
class User(UserBase, table=True):
    ...
    created_on: datetime.datetime = Field(default=datetime.datetime.now(datetime.timezone.utc), nullable=False)
...
class Item(ItemBase, table=True):
    ...
    owner_id: str | None = Field(default_factory=lambda: str(uuid.uuid4()), max_length=36, nullable=False)
    created_on: datetime.datetime | None = Field(default=datetime.datetime.now(datetime.timezone.utc), nullable=False)
    updated_on: datetime.datetime | None = Field(nullable=True, default=None)
...
class TokenRefresh(SQLModel):
    refresh_token: str = Field(default="", min_length=0, max_length=4000)
...
class IntegrationConfig(DefaultBase, table=True):
    ...
    updated_on: datetime.datetime | None = Field(
...
class Assignment(DefaultBase, table=True):
...
class TimeOffRequest(DefaultBase, table=True):
