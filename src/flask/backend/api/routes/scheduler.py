...
from backend.models import TimeOffRequestPublic
...
from backend.requests.assignment_request import (
    AssignmentCreate,
    AssignmentUpdate,
    BulkAssignRequest,
...
router = Blueprint("scheduler", __name__)
...
@router.route("/scheduler/time-off-requests", methods=["GET"])
...
def get_my_time_off():
    ...
    return jsonify(TimeOffRequestsPublic(data=[TimeOffRequestPublic.model_validate(a) for a in items], count=len(items)).model_dump())


@router.route("/scheduler/time-off-request", methods=["POST"])
@require_scope("member:limited")
...
@router.route("/scheduler/time-off-requests/<time_off_id>/approve", methods=["PATCH"])
...
@router.route("/scheduler/time-off-requests/<time_off_id>/decline", methods=["PATCH"])
@require_scope("scheduler:admin")
...
@router.route("/scheduler/time-off-requests/<time_off_id>", methods=["DELETE"])
@require_scope("member:limited")
...
@router.route("/scheduler/my-assignments", methods=["GET"])
@require_scope("member:limited")
def get_my_assignments():
    ...
    return jsonify(AssignmentsPublic(data=[AssignmentPublic.model_validate(a) for a in assignments], count=len(assignments)).model_dump())


@router.route("/scheduler/calendar", methods=["GET"])
@require_scope("member:limited")
...
@router.route("/scheduler/calendar-with-names", methods=["GET"])
@require_scope("member:limited")
def get_calendar_with_names():
    _tag = "scheduler"
    session = get_db()
    _ = get_current_user()
    start_date = request.args.get("start_date")
        ...
        enriched.append(
            EnrichedAssignment(
                **AssignmentPublic.model_validate(a).model_dump(),
                ...
                user_full_name=(user.full_name if user and user.full_name else (user.email if user else "unknown")),
            )
        )
    return jsonify(EnrichedAssignmentsResponse(data=enriched, count=len(enriched)).model_dump())


@router.route("/scheduler/my-calendar", methods=["GET"])
@require_scope("member:limited")
def get_my_calendar():
    _tag = "scheduler"
    session = get_db()
...
@router.route("/scheduler", methods=["GET"])
@require_scope("scheduler:admin")
def list_assignments(skip: int = 0, limit: int = 100):
    ...
    return jsonify(AssignmentsPublic(data=[AssignmentPublic.model_validate(a) for a in assignments], count=total_count).model_dump())


@router.route("/scheduler/<assignment_id>", methods=["GET"])
@require_scope("scheduler:admin")
...
@router.route("/scheduler", methods=["POST"])
@require_scope("scheduler:admin")
def create_assignment():
    ...
    repo = AssignmentRepository(session=session)
    conflicts = repo.check_conflicts(user_id=assignment_in.user_id, event_date=assignment_in.event_date)
    if conflicts:
        conflict_ids = [c.id for c in conflicts]
        conflict_details = "\n".join(f"  - {c.type}: {c.role} on {c.event_date.strftime('%Y-%m-%d')}" for c in conflicts)
        return (
            jsonify(
                {
...
@router.route("/scheduler/<assignment_id>", methods=["PATCH"])
@require_scope("scheduler:admin")
...
@router.route("/scheduler/<assignment_id>", methods=["DELETE"])
@require_scope("scheduler:admin")
...
@router.route("/scheduler/bulk", methods=["POST"])
@require_scope("scheduler:admin")
def bulk_assign():
    _tag = "scheduler"
    session = get_db()
    ...
    for entry in bulk_in.entries:
        ...
        if conflicts_check:
            conflicts.append(
                {
                    "user_id": entry.user_id,
                    "message": "User already has an assignment on " + bulk_in.event_date.strftime("%Y-%m-%d"),
