...
from backend.requests.integration_request import (
    CredentialUpdate,
    IntegrationCreate,
    IntegrationUpdate,
    TestConnectionRequest,
)
from backend.responses.integration_response import (
    IntegrationConfigPublic,
    IntegrationConfigPublicWithCreds,
    IntegrationsPublic,
    TestConnectionResponse,
)
from backend.services.integration_service import KNOWN_INTEGRATIONS, IntegrationService

router = Blueprint("integrations", __name__)
...
def _mask_credentials(creds):
    if not creds:
        return {}
    return {field: f"****{value[-4:]}" if len(value) > 4 else "****" for field, value in creds.items()}


@router.route("/integrations", methods=["GET"])
...
@router.route("/integrations/status")
...
@router.route("/integrations/<integration_id>", methods=["GET"])
@require_scope("integrations:admin")
...
@router.route("/integrations", methods=["POST"])
@require_scope("integrations:admin")
...
@router.route("/integrations/<integration_id>", methods=["PUT"])
@require_scope("integrations:admin")
...
@router.route("/integrations/<integration_id>/credentials", methods=["PATCH"])
@require_scope("integrations:admin")
...
@router.route("/integrations/<integration_id>", methods=["DELETE"])
@require_scope("integrations:admin")
...
@router.route("/integrations/test-connection", methods=["POST"])
@require_scope("integrations:admin")
def test_connection():
    _tag = "integrations"
    session = get_db()
...
@router.route("/integrations/sync-status/<integration_id>", methods=["POST"])
@require_scope("integrations:admin")
...
@router.route("/integrations/pre-seed", methods=["POST"])
@require_scope("integrations:admin")
