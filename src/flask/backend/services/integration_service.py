...
from backend.repositories.integration_repo import IntegrationConfigRepository
...
class IntegrationService:
    ...
    def get_by_type(self, type: str) -> IntegrationConfig | None:
        return self.repo.get_by_type(type)
    ...
    def update(self, db_item: IntegrationConfig, data: dict) -> IntegrationConfig:
        return self.repo.update(db_item, data)

    def update_credentials(self, db_item: IntegrationConfig, credentials: dict[str, str]) -> IntegrationConfig:
        iv, blob = EncryptionHelper.encrypt(json.dumps(credentials))
        ...
        self.repo.session.refresh(db_item)
        return db_item

    def delete(self, db_item: IntegrationConfig) -> None:
        self.repo.delete(db_item)

    def get_credentials(self, integration: IntegrationConfig) -> dict[str, str] | None:
        if not integration.cred_encrypted_blob or not integration.cred_encrypted_iv:
            return None
        plaintext = EncryptionHelper.decrypt(integration.cred_encrypted_iv, integration.cred_encrypted_blob)
        result: dict[str, str] = json.loads(plaintext)
        return result

    def test_connection(self, type: str, credentials: dict[str, str], config: dict | None = None) -> dict[str, Any]:
        handler = _CONNECTION_HANDLERS.get(type)
        if not handler:
            return {"success": False, "status": "error", "message": f"No handler for type '{type}'"}
        result = handler(credentials, config)
        return result

    def sync_status(self, integration: IntegrationConfig, status: str) -> IntegrationConfig:
        integration.status = status
        integration.last_synced_at = __import__("datetime").datetime.now(__import__("datetime").timezone.utc)
        self.repo.session.add(integration)
        self.repo.session.commit()
        self.repo.session.refresh(integration)
        return integration


@register_connection_handler("stripe")
def _test_stripe(credentials: dict[str, str], config: dict | None) -> dict[str, Any]:
    import stripe
...
def _test_twilio(credentials: dict[str, str], config: dict | None) -> dict[str, Any]:
    try:
...
def _test_sendgrid(credentials: dict[str, str], config: dict | None) -> dict[str, Any]:
...
def _test_youtube(credentials: dict[str, str], config: dict | None) -> dict[str, Any]:
    api_key = credentials.get("api_key", "")
    if not api_key:
        return {"success": False, "status": "error", "message": "Missing api_key"}
    import httpx
...
def _test_facebook(credentials: dict[str, str], config: dict | None) -> dict[str, Any]:
...
def _test_spotify(credentials: dict[str, str], config: dict | None) -> dict[str, Any]:
